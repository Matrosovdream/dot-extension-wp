...
final class DotFrmPhotosPageShortcode {
        ...
        <div class="ffda-confirm-backdrop" id="faipDenyModal">
            <div class="ffda-confirm-box">
                ...
                <div class="ffda-confirm-text">
                    Select reason. This will set:
                    <div class="faip-muted" style="margin-top:6px;">
                        Field <code>662</code> = checked, Field <code>663</code> = reason, Field <code>224</code> = unchecked
                    </div>
                </div>

                <div class="ffda-rate-group">
                    <div class="ffda-rate-group-title">Reason</div>
                    <select id="faipDenyReason" style="width:100%; border:1px solid #d0d7de; border-radius:8px; padding:8px 10px; font-size:14px;">
                        <option value="">— Select —</option>
                        <option value="Blurry / out of focus">Blurry / out of focus</option>
                        <option value="Wrong document / wrong person">Wrong document / wrong person</option>
                        <option value="Cut off / missing corners">Cut off / missing corners</option>
                        <option value="Too dark / glare">Too dark / glare</option>
                        <option value="Not readable">Not readable</option>
                        <option value="Other">Other</option>
                    </select>
                    <div id="faipDenyOtherWrap" style="display:none; margin-top:10px;">
                        <input id="faipDenyOther" type="text" placeholder="Type custom reason…" style="width:100%; border:1px solid #d0d7de; border-radius:8px; padding:8px 10px; font-size:14px;">
                    </div>
                </div>
    ...
    public function ajax_ai_fix_photo(): void {
        ...
        try {
            ...
            if ( ! function_exists('geminiTest') ) {
                wp_send_json([ 'ok' => false, 'error' => 'geminiTest() not found' ], 500);
            }

            $res = geminiTest($imagePath);
            if ( ! is_array($res) ) {
            ...
            if ( $final_url === '' ) {
                wp_send_json([ 'ok' => false, 'error' => 'final_url is empty', 'data' => $data ], 500);
            }
