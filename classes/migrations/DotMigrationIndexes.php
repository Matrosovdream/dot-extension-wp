...
final class DotMigrationIndexes {
    ...
    private const VERSION = '2026-01-13-1';
    ...
    public static function up(): void {
        ...
        $wpdb->query("ANALYZE TABLE `{$payments_authnet}`, `{$payments_failed}`, `{$refunds_authnet}`");
    }
    ...
    private static function ensure_index(string $table, string $index_name, string $alter_sql): void {
        ...
        if ($result === false) {
            error_log('[DotMigrationIndexes] Failed: ' . $alter_sql . ' | Error: ' . $wpdb->last_error);
        } else {
            error_log('[DotMigrationIndexes] Added index ' . $index_name . ' on ' . $table);
        }
    }
}
